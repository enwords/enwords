...
CSV.foreach(Rails.root.join('db', 'seeds_data', 'sentences.txt'), :col_sep => "\t", :headers => false,
            :encoding => 'utf-8').each do |row|
  sql = "insert into sentences (id, language_id, sentence) values (#{ row[0]}, #{ row[1]}, " + "'" + row[2].gsub("'", "''").gsub('"', '""') + "')" #SET Lang ID!
  ActiveRecord::Base.connection.execute(sql)
end
