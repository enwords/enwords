class User::CreateFromOmniauth < ActiveInteraction::Base
  ...
  def execute
    ...
    when existing_user.present?
      ...
      user = User.create(email:    auth_params.dig('info', 'email') ||
  ...
  def existing_user
    @_existing_user ||= (user || User.find_by(email: auth_params.dig('info', 'email')))
  end
end
